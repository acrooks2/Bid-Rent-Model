...
globals[
 ...
 sorted-assec-list ;; Reports a list of centers accessabilty from center 1, sorted from most to least accessability.
...
patches-own[
acces ;accessibility of a patch
...
reset-ticks
  ...
  if model_type = "regular"[
    set rings-data gis:load-dataset "data/rings.shp"]

  if model_type = "new center"[
...
;;what happens each tick of the model
to go
  tick
  reset-age-groups
...
;;Setting up the space requirements for the agents.
to set-space
...
;;how agents search for a new location
to search-new-area
 set areas-changed nobody
 let moved-in-this-round false
 foreach sorted-assec-list [ ?1 ->
   if moved-in-this-round = false [
     if ([cost] of ?1 * space) <= income [
       let target nobody
       repeat 50 [
         let try-target one-of [neighborhood] of ?1
         ask try-target [
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "clear-plot\nif ticks > 0[foreach sort [income] of turtles [ ?1 -> plot ?1 ]]"
...
402
593
Step 4: Press Setup, once model is initialized, either step through, or let the simualtion run 
11
0.0
1
...
843
572
Employers are Blue, Residents are Yellow
11
0.0
1
...
422
130
Step 2: Enter the number of Employers you want and the space they want. 
11
0.0
1
...
396
287
Note, the model assumes industy needs more space than commerce. 
11
0.0
1
...
378
353
Step 3: Enter the number of Residents you want and the space they want (note residents are broken into three age ranges (young, middle and old). 
11
0.0
1
...
391
492
Step 4: Chose how large of a search radius a agent can seach in
11
0.0
1
...
410
694
Step 5: As the model is running you can chose to add new agnts and see how this displaces other agents. 
11
0.0
1
...
890
621
Press this button if you want to see the accessibility surface.
11
0.0
1
...
866
677
Press this button if you want to see the just a black background (which is the default).
11
0.0
1
...
In this simple model there are two types of agents (breeds), employers and residents. Employment is broken down by three broad types, industry, service and commerce. Each employment type have their own specific space needs (e.g. industry might need more space than say commerce). While for residents, here they are broken down by rough age groups (young, middle and old), again different groups might have different space needs. Each tick of the models the agents can move to find the most accessible area they can afford.  This is outlined in Figure 1. 

![alt text](file:data/BasicSearching.png)
Figure 1: Basic model logic

## HOW TO USE IT

See the Steps in the Interface tab.

## THINGS TO NOTICE and TRY

As the model is running, add new employer agents, and you will observe how this pushes the residents further away from the central business district and eventually, the residents are forced to leave the system. 

## EXTENDING THE MODEL

This model could be extended in numerous ways. Easy extensions would be to color the agents based on their type, or giving the agents different preferences for locations. One could also explore how the effect of altering how often they could move. Adding more centers etc… Morover, you could link residents to employment and see what happens if employment locations change etc.  

## NETLOGO FEATURES

Model uses NetLogo’s GIS extension to create the world. See https://ccl.northwestern.edu/netlogo/docs/gis.html for further details.
